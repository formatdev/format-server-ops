...
## 2026-09-05 - Traefik Patch Upgrade

Date: 2026-09-05 09:34 CEST

Maintainer: Codex with Peter

Stack version before: `traefik:3.7.11`

Stack version after: `traefik:3.7.13`

Checks:

- Container and image metadata checked: OK. `traefik_traefik` is `1/1`, and the live image and stack label both report `traefik:3.7.13`.
- Latest release checked: OK. `v3.7.13` was the current upstream release and Docker official image during this run.
- Provider, network, ports, and dashboard policy checked: OK. Swarm routing remained on `proxy`, HTTPS remained on `443/tcp` and `443/udp`, and the dashboard remained disabled.
- Routed app smoke tests checked: OK. All seven sampled routes returned their expected responses after dependent services recovered.
- Logs reviewed: OK. Startup showed the familiar temporary missing-middleware lines; the fresh post-convergence error sample was quiet.
- Update applied: Yes.
- Notes: The expected single-node host-mode port `443` replacement wait occurred and then converged normally.
...
Stack version before: `traefik:3.7.8`

Stack version after: `traefik:3.7.10`

Checks:

- Container health checked: OK. `traefik_traefik` is `1/1`.
- Running image checked: OK. Live service uses `traefik:3.7.10`.
- Stack image metadata checked: OK. `com.docker.stack.image` reports `traefik:3.7.10`.
- Latest Traefik release checked: OK. GitHub latest release observed during this run is `v3.7.10`.
- Provider health checked: OK. Swarm provider remained active on the `proxy` network.
- Network membership checked: OK. Routed services remained attached and recovered after update.
- Ports checked: OK. HTTPS remained published on `443/tcp` and `443/udp`.
- Logs reviewed: OK. Startup showed the familiar temporary missing middleware lines, but the fresh post-convergence filtered sample was quiet.
- Routed app smoke tests checked: OK. Local host-header probes for representative routes returned expected responses.
- Dashboard/API policy checked: OK. The dashboard remains intentionally disabled.
- Update applied: Yes.
- Notes: The single-node host-mode `443` replacement wait occurred again during update and then converged cleanly.
...
## 2026-07-19 - Traefik 3.7 Line Upgrade

Date: 2026-07-19 09:50 CEST

Maintainer: Codex with Peter

Stack version before: `traefik:3.6.23`

Stack version after: `traefik:3.7.8`

Checks:

- Container health checked: OK. `traefik_traefik` is `1/1`.
- Running image checked: OK. Live service uses `traefik:3.7.8`.
- Stack image metadata checked: OK. `com.docker.stack.image` reports `traefik:3.7.8`.
- Latest Traefik release checked: OK. GitHub latest release observed during this run is `v3.7.8`.
- Migration notes reviewed: OK. The live static args use the Swarm provider, entrypoints, ACME DNS challenge, API disabled, and access-log options; no required local config change was identified before the image update.
- Provider health checked: OK. Swarm provider remained active on the `proxy` network.
- Network membership checked: OK. Routed services remained attached and recovered after update.
- Ports checked: OK. HTTPS remained published on `443/tcp` and `443/udp`.
- Logs reviewed: OK. Startup showed the familiar temporary missing middleware lines, but the fresh post-convergence error/deprecation sample was quiet.
- Routed app smoke tests checked: OK. Local host-header probes for representative routes returned expected responses.
- Dashboard/API policy checked: OK. The dashboard remains intentionally disabled.
- Update applied: Yes.
- Notes: The single-node host-mode `443` replacement wait occurred again during update and then converged cleanly.
...
## 2026-07-19 - Traefik Patch Upgrade

Date: 2026-07-19 09:43 CEST

Maintainer: Codex with Peter

Stack version before: `traefik:3.6.22`

Stack version after: `traefik:3.6.23`

Checks:

- Container health checked: OK. `traefik_traefik` is `1/1`.
- Running image checked: OK. Live service uses `traefik:3.6.23`.
- Stack image metadata checked: OK. `com.docker.stack.image` reports `traefik:3.6.23`.
- Latest Traefik release checked: OK. Latest `3.6.x` patch observed during this run is `v3.6.23`; newer `3.7.x` releases exist and were not applied as a routine line jump.
- Provider health checked: OK. Swarm provider remained active on the `proxy` network.
- Network membership checked: OK. Routed services remained attached and recovered after update.
- Ports checked: OK. HTTPS remained published on `443/tcp` and `443/udp`.
- Logs reviewed: OK. Fresh post-convergence Traefik error sample was quiet.
- Routed app smoke tests checked: OK. Local host-header probes for representative routes returned expected responses.
- Dashboard/API policy checked: OK. The dashboard remains intentionally disabled.
- Update applied: Yes.
- Notes: The single-node host-mode `443` replacement wait occurred again during update and then converged cleanly.
- Follow-up: Plan a deliberate `3.7.x` line migration review; `3.6.x` is no longer the active support line.

## 2026-07-04 - Traefik Patch Upgrade

Date: 2026-07-04 17:52 CEST

Maintainer: Codex with Peter

Stack version before: `traefik:3.6.21`

Stack version after: `traefik:3.6.22`

Checks:

- Container health checked: OK. `traefik_traefik` is `1/1`.
- Running image checked: OK. Live service uses `traefik:3.6.22`.
- Stack image metadata checked: OK. `com.docker.stack.image` reports `traefik:3.6.22`.
- Latest Traefik release checked: OK. Latest `3.6.x` patch observed during this run is `v3.6.22`; newer `3.7.x` releases exist and were not applied as a routine line jump.
- Provider health checked: OK. Swarm provider remained active on the `proxy` network.
- Network membership checked: OK. Routed services remained attached and recovered after update.
- Ports checked: OK. HTTPS remained published on `443/tcp` and `443/udp`.
